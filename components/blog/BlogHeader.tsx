...
const BlogHeader: React.FC = () => {
  ...
  return (
    <header className="md:flex">
      <div
        className={`lower-glassmorphism bg-bgdark z-30 top-0 shadow-sm fixed duration-400 px-4 sm:px-8 h-16 w-full ${navClassList.join(
          " "
        )}`}
      >
        <div className="w-full h-full mx-auto max-w-6xl flex items-center justify-between">
          <Link
            href="/blog"
            className="after:content-['blog'] after:bg-bglight after:text-bgdark after:text-base after:px-2 after:inline-block after:rotate-12 after:absolute after:-right-12 hover:after:rotate-0 relative text-xl sm:text-2xl hover:text-carrigreen focus-visible:outline-carrigreen"
          >
            SatNaing
            <span className="text-carrigreen">.dev</span>
          </Link>
              ...
              <ul className="flex justify-evenly items-center py-1">
                {navLinks.map((navLink) => (
                  ...
                  >
                    <Link
                      href={navLink.url}
                      className={`text-sm md:text-lg flex flex-col items-center w-[4.5rem] md:w-auto md:mr-6 fill-textlight md:hover:text-carrigreen link-outline
                        ${
